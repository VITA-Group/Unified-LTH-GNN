...
def load_data(dataset_str: str, dataset_path: str = None):
    """
    ...
    """
    dataset_path = dataset_path if dataset_path is not None else "../dataset"

    names = ["x", "y", "tx", "ty", "allx", "ally", "graph"]
    objects = []
    for i in range(len(names)):
        with open(f"{dataset_path}/ind.{dataset_str}.{names[i]}", "rb") as f:
    ...
    # print(f"Shapes x {x.shape}, y {y.shape}, tx {tx.shape}, ty {ty.shape}, allx {allx.shape}, ally {ally.shape}")
    test_idx_reorder = parse_index_file(f"{dataset_path}/ind.{dataset_str}.test.index")
    ...
    # TODO: revise this validation splitting this version executes but is not learning
    # current validation split set at 50% of size from test split
    # it is not learning properly on generated grph 6 but seems to work when applied to cora
    val_split_value = int(0.5 * len(test_idx_range.tolist()))
    idx_val = test_idx_range[0:val_split_value].tolist()
    idx_test = test_idx_range[val_split_value:].tolist()
...
def load_adj_raw(dataset_str, dataset_path: str = None):
    names = ["x", "y", "tx", "ty", "allx", "ally", "graph"]
    objects = []
    for i in range(len(names)):
        with open(f"{dataset_path}/ind.{dataset_str}.{names[i]}", "rb") as f:
